...
class MySQLKerberosAuthPlugin(BaseAuthPlugin):
    ...
    def _acquire_cred_with_password(self, upn):
        ...
        try:
            ...
            raise errors.ProgrammingError(
                "Unable to acquire credentials with the given password: {}".format(err)
            )
    ...
    def _parse_auth_data(self, packet):
        ...
        spn = struct.unpack("<{}s".format(spn_len), packet[:spn_len])[0]
        packet = packet[spn_len:]

        realm_len = struct.unpack("<H", packet[:2])[0]
        realm = struct.unpack("<{}s".format(realm_len), packet[2:])[0]
    ...
    def auth_response(self, auth_data=None):
        ...
        if auth_data:
            try:
                ...
                raise InterruptedError("Invalid authentication data: {}".format(err))

        if spn is None:
            return self.prepare_password()

        upn = "{}@{}".format(self._username, realm) if self._username else None
        ...
        try:
            ...
            upn = "{}@{}".format(self._username, realm) if self._username else creds_upn
            ...
            if upn and self._password is not None:
                ...
                raise errors.InterfaceError(
                    "Unable to retrieve cached credentials error: {}".format(err)
                )
...
class MySQL_OCI_AuthPlugin(BaseAuthPlugin):
    ...
    def _get_private_key(self, key_path):
        ...
        try:
            ...
            raise errors.ProgrammingError(
                'An error occurred while reading the API_KEY from "{}": {}'.format(key_path, err))
                #" {}".format(err))
    ...
    def _get_valid_oci_config(self, oci_path=None, profile_name="DEFAULT"):
                ...
                try:
                    ...
                    if oci_config[req_key] \
                       and not req_keys[req_key](oci_config[req_key]):
                        error_list.append('Parameter "{}" is invalid'.format(req_key))
                except KeyError as err:
                    error_list.append('Does not contain parameter {}'.format(req_key))
        except (
        ...
        if error_list:
            raise errors.ProgrammingError(
                'Invalid profile {} in: "{}".  Errors found: {}'.format(profile_name, oci_path, error_list))
                #f" Errors found: {}".format(profile_name, oci_path, error_list))
